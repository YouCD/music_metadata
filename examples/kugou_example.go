...
func main() {
	// 创建酷狗音乐provider
	kugouProvider := provider.NewKugouProvider()

	ctx := context.Background()

	// 搜索歌曲
	fmt.Println("=== 搜索歌曲 ===")
	songs, err := kugouProvider.Search(ctx, "周杰伦 晴天")
	if err != nil {
	...
	if len(songs) > 0 {
		fmt.Println("=== 获取歌词 ===")
		firstSong := songs[0]
		lyrics, err := kugouProvider.GetLyrics(ctx, firstSong)
		if err != nil {
		...
		// 获取封面
		fmt.Println("=== 获取封面 ===")
		data, contentType, err := kugouProvider.GetCover(ctx, firstSong)
		if err != nil {
